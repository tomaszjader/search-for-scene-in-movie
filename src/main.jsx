...
function App() {
  ...
  const importYoutube = async event => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) throw new Error(data.error)
      setSource({ type: 'youtube', name: data.title, author: data.author, duration: data.duration, videoId: data.videoId })
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {!source ? <section className="source-stage">
        <form className="youtube-import" onSubmit={importYoutube}><div className="youtube-icon"><Play size={25} fill="currentColor" /></div><div className="youtube-copy"><span className="step-label">01 / LINK YOUTUBE</span><strong>Wklej link do filmu</strong></div><div className="url-field"><Link2 size={17} /><input type="url" value={youtubeUrl} onChange={event => setYoutubeUrl(event.target.value)} placeholder="https://youtube.com/watch?v=..." /><button disabled={!youtubeUrl.trim() || status === 'importing'}>{status === 'importing' ? <LoaderCircle className="spin" size={17} /> : 'Analizuj'}</button></div></form>
